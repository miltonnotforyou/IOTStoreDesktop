...
namespace IOTStore
{
    public partial class formRelCargo : MaterialSkin.Controls.MaterialForm
    {
        public formRelCargo()
        {
            InitializeComponent();
        ...
        private void btPesquisar_Click(object sender, EventArgs e)
        {
            ...
            switch (relatorio)
            {
                ...
                case "Data de Cadastro":
                    ...
                    classCargoBindingSource.DataSource = cCargo.RelatorioCargoDataCadastro(Convert.ToDateTime(dtpDataInicial.Text), Convert.ToDateTime(dtpDataFinal.Text));
                    //REFRESH NO FORMS
                    this.rptvCargo.RefreshReport();
                ...
                case "Status":
                    ...
                    {
                        classCargoBindingSource.DataSource = cCargo.RelatorioCargoStatus(0);
                    }
                    else
                    {
                        classCargoBindingSource.DataSource = cCargo.RelatorioCargoStatus(1);
                    }
                    //REFRESH NO FORMS
                    this.rptvCargo.RefreshReport();
        ...
        {
            if (MessageBox.Show("Deseja fechar o relatório de Cargos???", "IOT Store", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                this.Close();
            }
        }
