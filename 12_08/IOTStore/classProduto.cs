...
namespace IOTStore
{
    class classProduto
    {
        ...
        {
            string sql = $" SELECT produto.codigo_produto, produto.nome, produto.qtde_estoque, produto.preco_venda, produto.lucro, produto.status_promocao, produto.foto, marca.nome, categoria.nome, produto.data_cadastro" +
                         $" FROM produto " +
                         $" JOIN marca ON produto.codigo_marca = marca.codigo_marca " +
                         $" JOIN categoria ON produto.codigo_categoria = categoria.codigo_categoria " +
                         $" WHERE produto.status = 1 AND produto.codigo_categoria = {categoria} ORDER BY produto.nome; ";
        ...
        //MÉTODO PARA CARREGAR OS PORDUTOS NO FORM DE VENDAS

        public DataTable CarregarGridProduto(string nomec)
        {
            string query = $" SELECT produto.codigo_produto AS 'COD', produto.nome AS 'Produto', marca.nome AS 'Marca', produto.qtde_estoque AS 'Estoque', produto.preco_venda AS 'Valor' " +
                $" FROM produto " +
                $" JOIN marca ON produto.codigo_marca = marca.codigo_marca " +
                $" JOIN categoria ON produto.codigo_categoria = categoria.codigo_categoria" +
                $" WHERE produto.nome LIKE '%" + nomec + "%' AND produto.status = 1 ORDER BY produto.nome";
        ...
        //MÉTODO PARA ATUALIZAR QTDE ESTOQUE QUANDO UMA VENDA É FINALIZADA
        public bool AtualizaEstoque(int qtde, int cod)
        {
            string query = $"UPDATE produto SET qtde_estoque = {qtde} WHERE codigo_produto = {cod}";
        ...
        //METDO PARA LEVAR NOME DO PRODUTO PARA A LISTA DE ITENS
