...
namespace IOTStore
{
    class classCliente
    {
        ...
        {
            string sql = $"INSERT INTO cliente VALUES(0, '{nome}', '{nome_social}', '{data_nascimento.ToString("yyyy-MM-dd")}', '{sexo}', '{cpf}', '{rg}', '{endereco}', {numero}, '{complemento}', '{bairro}', '{cidade}', '{estado}', '{cep}', '{telefone_residencial}','{telefone_celular}', '{email}', '{senha}', 1, NOW());";
        ...
        {
            string sql = $"UPDATE cliente SET nome = '{nome}', nome_social = '{nome_social}', data_nascimento = '{data_nascimento.ToString("yyyy-MM-dd")}', sexo = '{sexo}', cpf = '{cpf}', rg = '{rg}', endereco = '{endereco}', numero = {numero}, complemento = '{complemento}', bairro = '{bairro}', cidade = '{cidade}', estado = '{estado}', CEP = '{cep}', telefone_residencial = '{telefone_residencial}', telefone_celular = '{telefone_celular}', email = '{email}'";

            // Só atualiza a senha no banco se o administrador digitou uma nova
            if (!string.IsNullOrEmpty(senha))
            {
                sql += $", senha = '{senha}'";
            }

            sql += $", status = {status} WHERE codigo_cliente = {codigo_cliente}";
        ...
        //MÉTODO PARA CARREGAR OS CLIENTES NO FORM DE VENDAS

        public DataTable CarregarGridCliente(string nomec)
        {
            string query = "SELECT codigo_cliente AS 'COD', nome AS 'Cliente', cpf FROM cliente WHERE nome LIKE '%" + nomec + "%' AND cliente.status = 1 ORDER BY cliente.nome";
            
            classConexao cConexao = new classConexao();
            return cConexao.RetornaDados(query);
        }

        //MÉTODO PARA CARREGAR OS Clientes NO Relatorio DE VENDAS

        public DataTable BuscarCliente()
        {
            string query = "SELECT codigo_cliente, nome FROM cliente WHERE cliente.status = 1 ORDER BY cliente.nome";
            classConexao cConexao = new classConexao();
