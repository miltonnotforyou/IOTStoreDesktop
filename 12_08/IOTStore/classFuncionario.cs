...
namespace IOTStore
{
    class classFuncionario
    {
        ...
        //MÉTODO PARA CARREGAR OS FUNCIONARIOS NO FORM DE VENDAS

        public DataTable BuscarFuncionario()
        {
            string query = "SELECT funcionario.codigo_funcionario, funcionario.nome FROM funcionario JOIN cargo On cargo.codigo_cargo = funcionario.codigo_cargo WHERE funcionario.status = 1 AND cargo.nome LIKE '%vendedor%' ORDER BY funcionario.nome";
            classConexao cConexao = new classConexao();
