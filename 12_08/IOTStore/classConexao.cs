...
using System.Windows.Forms; //para usar o MessageBox
...
namespace IOTStore
{
    class classConexao
    {
        ...
        {
            //WEB
            //string connectionString = "Server=10.209.60.1;Port=3307;Database=iot_store;Uid=root;Pwd=DgFyaQHg8uvD6gV6KoXVLnfdrqQQpU58EnYhWUOaiNWaSgrgncG0iPOlBDhi1G7T;SslMode=none;";
            string connectionString = "Server=144.22.205.233;Port=3307;Database=iot_store;Uid=root;Pwd=DgFyaQHg8uvD6gV6KoXVLnfdrqQQpU58EnYhWUOaiNWaSgrgncG0iPOlBDhi1G7T;SslMode=none;";
            //LOCAL
            ...
            if (c.State == ConnectionState.Closed || c.State == ConnectionState.Broken)
            {
                ...
                //c.ConnectionString = conexao; local
                c.ConnectionString = connectionString; //web
                //ABRIR A CONEXÃO 
                c.Open();
            }
        }
        ...
        public int ExecutaQuery(string query)
        {
            ...
            {
                erro = ex.Message;
                MessageBox.Show("Erro ao Gravar no BD: " + ex.Message, "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return 0;
            }
            finally
            {
        ...
        public DataTable RetornaDados(string query)
        {
            ...
            {
                erro = ex.Message;
                MessageBox.Show("Erro de Consulta no BD: " + ex.Message, "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return null;
            }
            finally
            {
        ...
        public int ExecutaQueryID(string query)
        {
            ...
            {
                erro = sqlex.Message;
                MessageBox.Show("Erro ao Gravar (ID) no BD: " + sqlex.Message, "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Desconectar();
                return 0;
            }
        }
